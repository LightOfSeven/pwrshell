...
if($BitlockerStatus.VolumeStatus -notlike "FullyEncrypted"){
    ...
    Write-Output -Message "Variables set, proceeding to check TPM availability and the System Drive readiness state, before applying Bitlocker"
    ...
    if ($WindowsVer -and $TPM -and $SystemDriveBitLockerRDY) {
        ...
        Write-Debug -Message "Bitlocker is already enabled for this computer, skipping and leaving error code as 0"
        }
    }
    else {
        Write-Error -Message "Computer $Hostname failed to apply Bitlocker due to Windows Version, Bitlocker drive readiness or TPM version being incorrect/incompatible"
        $CustomErrorCode = 11
    }
}
else{
    Write-Output "Bitlocker already active"
}
Exit $CustomErrorCode
